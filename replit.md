...
- **Self-Service Kiosk**: Full-screen `/kiosk` page for a lobby tablet — expected (pre-registered, today, pending) guests search their name, take their own webcam photo, self check-in, and the badge auto-prints (`printBadge()` ~700ms after the badge screen renders; silent printing needs a kiosk-mode browser flag like Chrome `--kiosk-printing`). Steps: welcome → name search → photo → badge, with idle auto-reset back to welcome (90s search/photo, 45s badge, 20s desk screen). Any watchlist match (bidirectional partial name match, block OR flag) halts self check-in with a generic "Please see the security desk" screen — never reveals watchlist status. Conversion is atomic (transaction claims the pending prereg via conditional UPDATE before inserting the guest, so simultaneous requests can't double-convert). Audited as operator "Self check-in (kiosk)" with `kiosk: true` metadata; fires the normal check-in email alert + known-guest upsert. Endpoints: `GET /kiosk/preregistrations?q=` (limited fields, today's pending only, ≥2 chars, max 8) and `POST /kiosk/checkin` — both requireAuth (any role incl. kiosk).
